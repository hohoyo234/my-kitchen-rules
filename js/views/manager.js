    ...
    function draw(){
      const cells = DAYS.map((d,di)=>{
        ...
        const chips = ds.map(s=>{ const st=staffOf(s.staffId);
          ...
          return `<span class="shift-chip ${cls}" draggable="true" data-id="${s.id}">${st.name} ${s.start}${reminded?' <span title="已送达上班提醒">🔔</span>':''}<span class="rm" data-rm="${s.id}">×</span></span>`; }).join('');
  ...
  async function swaps(c){
    ...
    U.qs('#sosBtn',c).onclick=()=>{
      ...
      U.modal('🆘 发布 SOS 紧急顶班',wrap,{actions:[{label:'推送给空闲员工',class:'btn-accent',onClick:async(cl)=>{
        ...
        if(MKR.notify&&MKR.notify.push) MKR.notify.push({role:'staff'}, '🆘 紧急顶班', title+' · 奖励 '+reward, 'sos');
